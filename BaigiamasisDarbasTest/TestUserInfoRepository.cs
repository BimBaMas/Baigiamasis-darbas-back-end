...
namespace BaigiamasisDarbasTest
{
    public class TestUserInfoRepository
    {
        ...
        {
            UserInfo userInfo = userInfoRepository.GetById(user.Id);
            Assert.NotNull(userInfo);
        }
        [Test]
        public void CreateNewUserInfoAndTestIfItsInDatabase()
        {
            userInfoDTO.UserId = user2.Id;
            UserInfo userInfo = userInfoRepository.Create(userInfoDTO);
            Assert.NotNull(userInfo);
        }
        [Test]
        public void TestUpdateNameNameAfterUpdateEqualsUpdated()
        {
            UserInfo userInfo = userInfoRepository.UpdateName(user.Id, "Updated");            
            Assert.AreEqual(userInfo.Name, "Updated");
        }
        [Test]
        public void TestUpdateSurnameSurnameAfterUpdateEqualsUpdated()
        {
            UserInfo userInfo = userInfoRepository.UpdateSurname(user.Id, "Updated");
            Assert.AreEqual(userInfo.Surname, "Updated");
        }
        [Test]
        public void TestUpdateEmailEmailAfterUpdateEqualsUpdated()
        {
            UserInfo userInfo = userInfoRepository.UpdateEmail(user.Id, "Updated");
            Assert.AreEqual(userInfo.Email, "Updated");
        }
        [Test]
        public void TestUpdatePhoneNoPhoneNolAfterUpdateEqualsUpdated()
        {
            UserInfo userInfo = userInfoRepository.UpdatePhoneNo(user.Id, "Updated");
            Assert.AreEqual(userInfo.PhoneNo, "Updated");
        }
        [Test]
        public void TestUpdatePersonalIdPersonalIdlAfterUpdateEqualsUpdated()
        {
            UserInfo userInfo = userInfoRepository.UpdatePersonalId(user.Id, "Updated");
            Assert.AreEqual(userInfo.PersonalId, "Updated");
        }
        [Test]
        public void TestDeleteDeletesOneRecord()
        {
            int countbefore = userInfoRepository.Get().Count;
            userInfoRepository.Delete(user3.Id);
            int countafter = userInfoRepository.Get().Count;
